...
export async function createCompletion(prompt: string, description: string) {
  if (!prompt || !description) {
    return { error: 'Prompt and description is required' }
  }
  ...
  const messages: any = [
    {
      role: 'user',
      content: `Write a blog post around 200 words about the following topic: "${prompt}", focusing on "${prompt}",  in markdown format.`
    }
  ]
